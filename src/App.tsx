...
export default function App() {
  ...
  const process = async () => { setStep('processing'); setProcessing(true); setTrades([]); let worker: Awaited<ReturnType<typeof createWorker>> | undefined;
    try { worker = await createWorker('eng'); const output: Trade[] = []; for (let index=0; index<images.length; index++) { const image = images[index]; setImages(prev=>prev.map((x,i)=>i===index?{...x,status:'processing'}:x)); try { const { data } = await worker.recognize(image.file); const useAi = aiReviewEnabled && aiReviewStatus === 'secure-ai'; const imageDataUrl = useAi ? await imageToDataUrl(image.file) : undefined; const parsed = await parseTrades(data.text, image.file.name, image.url, { aiReviewEnabled: useAi, imageDataUrl }); output.push(...parsed); setImages(prev=>prev.map((x,i)=>i===index?{...x,status:'done'}:x)); } catch { setImages(prev=>prev.map((x,i)=>i===index?{...x,status:'failed',error:'Extraction failed'}:x)); output.push({id:crypto.randomUUID(),sourceFileName:image.file.name,sourceUrl:image.url,confidence:emptyConfidence,needsReview:true}); } } setTrades(output); setActive(0); setStep('results'); }
  ...
  return <div className="app"><header><a className="brand" href="#top" onClick={()=>setStep('home')}><span>▦</span>{APP_NAME}</a><nav><a href="#how">How it works</a><a href="#privacy">Privacy</a><a href="#about">About</a></nav></header>
    ...
    <main id="top">
      ...
      {aiReviewEnabled && aiReviewStatus === 'secure-ai' && (
        <div className="notice" style={{marginTop:'12px'}}>
          <span>AI review is on — your screenshots are sent to the secure AI backend for analysis. Turn it off to keep everything on your device.</span>
        </div>
      )}
      {step==='home' && <section className="hero"><div className="eyebrow">PRIVATE • LOCAL-FIRST • NO SIGN-UP</div><h1>Turn your trade screenshots into an Excel journal</h1><p>Upload up to 15 screenshots from MT4, MT5 or TradingView. We’ll extract the trade data and create an organized Excel file for you.</p><div className="actions"><button className="primary" onClick={()=>input.current?.click()}>Upload Trade Screenshots <b>→</b></button><span>JPG, PNG, WEBP · Up to 15 images</span></div><div className="privacy-card">🔒 <div><strong>Local-first by default.</strong><br/>We never store your screenshots or trades. Turning on optional AI review sends the screenshot for analysis.</div></div><div className="steps"><div><b>1</b><strong>Upload screenshots</strong><span>Select your MT4, MT5 or TradingView images.</span></div><div><b>2</b><strong>Review extracted data</strong><span>Correct anything that needs attention.</span></div><div><b>3</b><strong>Download your journal</strong><span>Get a clean Excel workbook instantly.</span></div></div></section>}
